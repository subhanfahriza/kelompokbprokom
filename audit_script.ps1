...
foreach ($restaurant in $restaurants) {
    ...
    if ($distanceKm -gt $MAX_DISTANCE_KM) {
        ...
        $removedList += @{
            ...
            reason = "Outside 5km radius (actual: $distRounded km)"
        }
        $removedCount++
    } else {
...
# Continue from here
    ...
    if ($distanceKm -gt $MAX_DISTANCE_KM) {
        Write-Host "❌ REMOVE - ID $id`: $name" -ForegroundColor Red
        Write-Host "   Address: $address"
        Write-Host "   Distance: $([Math]::Round($distanceKm, 2)) km (exceeds $MAX_DISTANCE_KM km limit)"
        Write-Host ""
        ...
        $removedList += @{
            ...
            reason = "Outside 5km radius (actual: $([Math]::Round($distanceKm, 2)) km)"
        }
        $removedCount++
    } else {
        Write-Host "✓ KEEP - ID $id`: $name" -ForegroundColor Green
        Write-Host "   Address: $address"
        Write-Host "   Latitude: $lat | Longitude: $lng"
        Write-Host "   Distance: $([Math]::Round($distanceKm, 2)) km"
        Write-Host ""
...
$msg = "Verified " + [char]38 + " Kept: $verifiedCount"
